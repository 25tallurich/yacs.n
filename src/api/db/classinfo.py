class ClassInfo:
    ...
    def get_major_restricted_classes(self, semester=None, majors=None):
        if semester is not None and majors is not None:
            major_restricted_query = """
                ...
                ORDER BY
                    ...
                    c.level ASC
            """
            return self.db_conn.execute(major_restricted_query, [semester, majors], True)
        return None

    def get_classes_by_search(self, semester=None, search=None, comm_intensive=False, major_resticted=False, majors):
      if semester is not None:
        if(comm_intensive and major_resticted):
            return self.db_conn.execute("""
            WITH ts AS (
                SELECT
                    c.department,
